...
| Validation | A plain acceptance function in Application, returning a result | The API is an entry point to the business rules, not their owner. The rule takes content plus declared metadata and no ASP.NET Core types, so it runs from a test or a console app unchanged |
| Response ordering | **Validate the whole upload, then write the response** | The two cannot interleave: once a response byte is flushed the status is committed, so a late rejection cannot become a 415. See ADR-0009 |
| Format dispatch | **None.** One `IFileMutator`, one acceptance function | Dropped after review. With a single accepted format a registry restates the acceptance rule in a second place, and `MutationCapability` was self-reported metadata the type system never enforced. Dispatch goes in when a second format exists |
...
| Business rules are not the API's | Acceptance rules live in Application and take no HTTP types | The endpoint parses multipart, calls the rule, maps the result to a status code. Everything that decides *whether a file is acceptable* is reachable without an HTTP request, which is what makes it testable and reusable |
