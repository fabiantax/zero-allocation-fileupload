...
| Stryker.NET mutation testing | Not implemented | Branch coverage is gated in CI and every gate has been seen to fail on purpose; mutation testing adds a slower second CI dimension whose marginal signal over the existing RED/GREEN culture does not justify its runtime at this codebase size. Revisit if the suite grows substantially |
