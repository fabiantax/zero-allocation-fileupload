...
Those belong to `../FileMutation.Api`. The service is registered scoped:
stateless today, but scoped survives the next per-request dependency (a DbContext would be
captive in a singleton). Per-call data lives on the result (`IAsyncDisposable`, disposed by the
caller) and in local pipes.
